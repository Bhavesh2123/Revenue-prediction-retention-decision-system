...
def train_model(model_df):
    ...
    param_distributions = {
        "n_estimators":     [100, 200, 300, 500],   # FIX: removed 3000 — was a typo
        "max_depth":        [3, 4, 5, 6, 8],
        "learning_rate":    [0.01, 0.03, 0.05, 0.1, 0.2],
        "subsample":        [0.6, 0.7, 0.8, 0.9, 1.0],
        "colsample_bytree": [0.6, 0.7, 0.8, 0.9, 1.0],  # FIX: was "comsample_bytree" (typo)
    ...
    print("[ltv_model] Starting RandomizedSearchCV — this may take a few minutes...")
    ...
    metrics = {
        ...
        "cv_best_rmse": round(float(-search.best_score_), 4),
    }
    ...
    # FIX: now correctly returns (best_model, metrics) as a tuple
    # Callers must unpack: model, metrics = train_model(model_df)
